...
BACKUP_FILE="$BACKUP_DIR/bdECommerce.sql"
...
# Supprimer toutes les vues de la base de données existante
echo "Suppression de toutes les vues de la base de données existante $DB_NAME..."
VUES=$(mysql -u $DB_USER -p$DB_PASSWORD -N -e "SELECT table_name FROM information_schema.views WHERE table_schema = '$DB_NAME';")
for VUE in $VUES; do
  mysql -u $DB_USER -p$DB_PASSWORD -e "DROP VIEW IF EXISTS $VUE;" $DB_NAME
done
